...
void calculateHeight(double initialHeight, int elapsedSeconds)
{
	double currentHeight{};
	currentHeight = initialHeight - (GRAVITY_CONSTANT * (elapsedSeconds * elapsedSeconds) / 2);
	if (currentHeight > 0)
		std::cout << "At " << elapsedSeconds << " seconds, the ball is at height: " << currentHeight << " meters\n";
	else
		std::cout << "At " << elapsedSeconds << " seconds, the ball is on the ground.\n";
}
...
int main()
{
	...
	calculateHeight(initialHeight, 0);
	calculateHeight(initialHeight, 1);
	calculateHeight(initialHeight, 2);
	calculateHeight(initialHeight, 3);
	calculateHeight(initialHeight, 4);
	calculateHeight(initialHeight, 5);
}